...
const FieldsWrapper: React.FC<Props> = ({ applyLogic, isActive }) => {
  ...
  const { answers, currentBlockAnswer } = useSelect((select) => {
    return {
      ...
      currentBlockAnswer: currentBlockId
        ? select("quillForms/renderer-core").getFieldAnswerVal(currentBlockId)
        : null,
    };
  });
  ...
  const isLastField =
    walkPath?.length && currentBlockId === walkPath[walkPath.length - 1].id;
  ...
  const swipingHandler = (
    ...
    if (
      canSwipePrev &&
      ((delta < -50 && !touch) ||
        (touch && delta > 50 && (e as SwipeEventData).dir === "Down")) &&
      !isFirstField
    ) {
      // Scroll up
      ...
      canSwipeNext &&
      ((delta < -50 && touch && (e as SwipeEventData).dir === "Up") ||
        (!touch && delta > 50)) &&
      !isLastField
    ) {
      lastScrollDate = new Date().getTime();
