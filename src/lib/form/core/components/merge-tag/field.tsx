...
const FieldMergeTag: React.FC<Props> = ({ modifier }: Props) => {
  const { fieldMergeTagValue } = useSelect((select) => {
    const fieldVal = select("quillForms/renderer-core").getFieldAnswerVal(
      modifier
    );
    const blockTypes: any = select("quillForms/blocks").getBlockTypes();
  ...
  return (
    <span className="renderer-core-field-merge-tag">{fieldMergeTagValue}</span>
  );
};
