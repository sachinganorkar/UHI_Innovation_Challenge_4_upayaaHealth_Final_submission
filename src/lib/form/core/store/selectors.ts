...
import type { FormBlock, FormBlocks } from "../../types";
 ...
 */
export function getBlockById(state: State, id: string): FormBlock | undefined {
  return state.swiper.walkPath.find((block: { id: string }) => block.id === id);
}
...
export const getEditableFieldsInCurrentPath = (state: State) => {
  ...
  return state.swiper.walkPath.filter((block: { name: any }) => {
...
export function getBlockCounterValue(
  ...
  const counterValue = editableFields.findIndex(
    (editableField: { id: string }) => editableField.id === id
  );
 ...
 */
export function getInvalidAnswers(state: State): Partial<RendererAnswersState> {
  return pickBy(state.answers, (o) => o.isValid === false);
}
